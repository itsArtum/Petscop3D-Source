...
void main()
{
    ...
    gl_Position.x = (floor(gl_Position.x/1.6)*1.6);
    gl_Position.y = (floor(gl_Position.y/1.6)*1.6);
    v_vColour = in_Colour;
...
void main()
{
    ...
    */
    ...
    //float dist = 1.0 - clamp( (distance(v_vPosWorld.xy,pos.xy)/radius),0.0,1.0);
    
    
vec2 relative = v_vPosWorld.xy - pos.xy;
float relativeDot = relative.x * relative.x + relative.y * relative.y;
float dist = 1.0 - clamp(sqrt(relativeDot) / radius, 0.0, 1.0);
