...
while(string_length(_strab) > 0)
{
       ...
       if string_copy(str_command[i],0,4) = "rgb(" or string_copy(str_command[i],0,15) = "make_color_rgb("{
            ...
            draw_set_color(uc)
            draw_set_alpha(ua)
            draw_line_width(_x+str_length,_y+str_height+string_height(str_part[i])+uy,_x+str_length+string_width(str_part[i]),_y+str_height+string_height(str_part[i])+uy,uwidth)
            ...
            draw_sprite(img,index,_x+str_length,_y+str_height-img_height/2+string_height(str_part[i])/2)     
            ...
            draw_sprite_ext(img,index,_x+img_x+str_length-img_xorg+img_width/2-((img_width/2)*img_xscale),_y+img_y+str_height-(img_height/2)+(string_height("ABCabc")/2)-img_yorg+img_height/2-((img_height/2)*img_yscale),img_xscale,img_yscale,img_rot,draw_get_color(),img_alp)     
       ...
       xoffset = string_width(str_part[i])/2
       yoffset = string_height(str_part[i])/2
       rad = degtorad(angle)
       xorg = cos(rad)*xoffset + sin(rad)*yoffset - xoffset - (string_width(str_part[i])/2)+((string_width(str_part[i])/2)*(_xscale*_xs))
       yorg = -sin(rad)*xoffset + cos(rad)*yoffset - yoffset - (string_height(str_part[i])/2)+((string_height(str_part[i])/2)*(_yscale*_ys))
       
       draw_set_color(d_color)
       draw_text_transformed_colour(_x+_posx+str_length-xorg,_y+(_posy)+str_height-yorg,str_part[i]+new_string,_xscale*_xs,_yscale*_ys,angle,d_color,d_color,box_color,box_color,_alpha)
       if mark{
            m_prea = draw_get_alpha();
            m_prec = draw_get_color();
            draw_set_alpha(m_a*_alpha);
            draw_set_color(m_c);
            draw_rectangle(_x+str_length,_y+str_height,_x+str_length+string_width(str_part[i]),_y+str_height+string_height(str_part[i]),false)
            draw_set_alpha(m_prea);
            draw_set_color(m_prec);
       }
       str_length += (string_width(str_part[i])+string_width(string_repeat("A",string_length(secret_hide))))
       _str[0] += str_part[i]+secret_hide
       if str_check{
          if _str[1] < string_width(_str[0]){
             _str[1] = string_width(_str[0])
          }
          str_length = 0 
          if img_h > string_height("A")+1{
            str_height += img_h
            img_h = 0
          }else{
            str_height += string_height("A")+1
            img_h = 0
          }
          _str[0] += "#"
       }
...
_str[2] = str_height+(string_height("A"))
